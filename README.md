...
| `random_agent.QuaverRandomAgent` | FP control — activity vs outcome | Drives the world's real tools with arbitrary args (generic; no hardcoded names), then writes a well-formed but WRONG final state — catches rubrics that reward activity over outcome |
| `forbidden_subprocess.QuaverForbiddenSubprocess` | FP control — sanctioned-interface bypass | Writes success-shaped state via raw interpreters/network/installs/background subprocess instead of the tools; floored by execution verification (no trajectory) and safety checks. Complements `pattern_1` (which breaks *out* of the container) by staying in and going *around* the tools |
| `shortcut_hunter.QuaverShortcutHunter` | FP control — answer-leak | Greps the checkers/fixtures for expected literals and hardcodes them into output; the runtime complement to a static answer-leak check |
